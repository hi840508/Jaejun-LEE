...
// 🚀 [자산 완전 보존] 데이터 소실을 막기 위해 프로젝트 폴더 외부인 홈디렉토리에 원장을 단독 배치합니다.
...
app.post('/api/auth/verify', (req, res) => {
    db.get(`SELECT * FROM users WHERE name = ?`, [req.body.name], (err, row) => {
        ...
        if (row) {
            ...
            else res.status(401).json({ exists: true, error: "자격 증명 일치하지 않음" });
        } else {
...
app.post('/api/user/update', (req, res) => {
    db.run(`UPDATE users SET bank = ?, account = ?, profilePic = ? WHERE name = ?`, [req.body.bank, req.body.account, req.body.profilePic, req.body.name], () => res.json({success: true}));
});
...
app.post('/api/transfer', (req, res) => {
    ...
    db.get(`SELECT balance FROM users WHERE name = ?`, [req.body.sender], (err, sRow) => {
        if (!sRow || sRow.balance < amount) return res.status(400).json({ error: "가용 한도 부족" });
        db.serialize(() => {
...
app.post('/api/check/issue', (req, res) => {
    ...
    db.get(`SELECT balance FROM users WHERE name = ?`, [req.body.issuer], (err, row) => {
        if (!row || row.balance < amount) return res.status(400).json({ error: "한도 한계 초과" });
...
app.post('/api/check/redeem', (req, res) => {
    ...
    db.get(query, params, (err, row) => {
        if (!row) return res.status(404).json({ error: "무효한 인증 코드" });
        if (signature && !verifyECCInverseSignature(row.id, row.secretKey, row.amount, signature)) return res.status(401).json({ error: "무결성 서명 실패" });
...
app.post('/api/buy', (req, res) => {
    ...
    db.get(`SELECT balance FROM users WHERE name = ?`, [req.body.buyer], (err, row) => {
        if (!row || row.balance < amount) return res.status(400).json({ error: "원장 잔액 부족" });
        db.serialize(() => {
