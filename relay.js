...
// 글로벌 타원곡선 키페어 생성 (모든 OpenSSL 환경에서 100% 호환되는 prime256v1 규격 적용)
const { publicKey, privateKey } = crypto.generateKeyPairSync('ec', { namedCurve: 'prime256v1' });

// 독자적 타원곡선 역산(ECC Inverse) 해시 엔진
...
const db = new sqlite3.Database(path.join(__dirname, 'commerce_master_ultimate_v3.db'));
...
app.post('/api/auth', (req, res) => {
    ...
    db.get(`SELECT * FROM users WHERE name = ?`, [name], (err, row) => {
        ...
        if (row) {
            ...
            db.run(`INSERT INTO users (name, password, bank, account, balance) VALUES (?, ?, ?, ?, 10000)`, [name, password, bank, account], (err2) => {
...
app.post('/api/user/profile', (req, res) => {
    db.run(`UPDATE users SET profilePic = ? WHERE name = ?`, [req.body.profilePic, req.body.name], (err) => {
        if (err) return res.status(500).json({ error: err.message });
...
app.post('/api/deposit/request', (req, res) => {
    db.run(`INSERT INTO deposits (user_name, sender_name, amount, status, date) VALUES (?, ?, ?, '대기', ?)`, 
        [req.body.userName, req.body.senderName, req.body.amount, new Date().toLocaleString('ko-KR')], (err) => { 
            if (err) return res.status(500).json({ error: err.message });
            res.json({ success: true }); 
        });
});
...
app.post('/api/withdraw/request', (req, res) => {
    db.serialize(() => {
        ...
        db.run(`INSERT INTO withdrawals (name, amount, status, date) VALUES (?, ?, '대기', ?)`, 
            [req.body.name, req.body.amount, new Date().toLocaleString('ko-KR')], (err) => { 
                if (err) return res.status(500).json({ error: err.message });
                res.json({ success: true }); 
            });
    });
...
app.post('/api/admin/approve', (req, res) => {
    ...
    if(type === 'deposit_direct') {
        ...
        db.serialize(() => {
            db.run(`INSERT INTO qr_checks (id, amount, issuer, secretKey, eccSignature, is_used, date) VALUES (?, ?, ?, ?, ?, 0, ?)`, [checkId, amount, adminName, secretKey, signature, date]);
            db.run(`UPDATE deposits SET status = '승인_QR송부' WHERE id = ?`, [id]);
            
            const chatRoom = `room_msg_${[userName, adminName].sort().join('_')}`;
            const qrPayload = `[META_QR]${checkId}|${secretKey}|${signature}`;
            db.run(`INSERT INTO chats (roomId, sender, message, date) VALUES (?, ?, ?, ?)`, [chatRoom, adminName, qrPayload, date], () => {
                res.json({ success: true });
                io.to(chatRoom).emit('receive_message', { roomId: chatRoom, sender: adminName, message: qrPayload, date });
            });
        });
    } else if (type === 'withdraw') {
        db.run(`UPDATE withdrawals SET status = '완료' WHERE id = ?`, [id], () => res.json({ success: true }));
    }
});
...
app.post('/api/transfer', (req, res) => {
    ...
    db.get(`SELECT balance FROM users WHERE name = ?`, [sender], (err, sRow) => {
        if (err) return res.status(500).json({ error: err.message });
        if (!sRow || sRow.balance < amount) return res.status(400).json({ error: "가용 자산이 부족합니다." });
        db.serialize(() => {
            ...
            db.run(`INSERT INTO transfers (sender, receiver, amount, date) VALUES (?, ?, ?, ?)`, 
                [sender, receiver, amount, new Date().toLocaleString('ko-KR')], () => res.json({ success: true }));
        });
    });
});
...
app.post('/api/check/issue', (req, res) => {
    ...
    db.get(`SELECT balance FROM users WHERE name = ?`, [issuer], (err, row) => {
        if (err) return res.status(500).json({ error: err.message });
        if (!row || row.balance < amount) return res.status(400).json({ error: "발행 한도를 초과했습니다." });
...
app.post('/api/check/redeem', (req, res) => {
    ...
    db.get(query, params, (err, row) => {
        if (err) return res.status(500).json({ error: err.message });
        if (!row) return res.status(404).json({ error: "이미 회수되었거나 무효한 핀(PIN)입니다." });
        
        if (signature && !verifyECCInverseSignature(row.id, row.secretKey, row.amount, signature)) {
            return res.status(401).json({ error: "ECC 역산 증명에 실패했습니다. (위조 위험)" });
        }
        db.serialize(() => {
...
app.post('/api/store/create', (req, res) => {
    ...
    db.run(`INSERT INTO stores (id, name, owner, logo, status) VALUES (?, ?, ?, ?, 'active')`, [storeId, name, owner, logo], (err) => {
        if (err) return res.status(500).json({ error: err.message });
        res.json({ success: true });
    });
});
...
app.post('/api/buy', (req, res) => {
    ...
    db.get(`SELECT balance FROM users WHERE name = ?`, [buyer], (err, row) => {
        if (err) return res.status(500).json({ error: err.message });
        if (!row || row.balance < amount) return res.status(400).json({ error: "자산 한도를 초과했습니다." });
        db.serialize(() => {
            ...
            db.run(`INSERT INTO transactions (buyer, seller, productName, amount, date) VALUES (?, ?, ?, ?, ?)`, 
                [buyer, seller, productName, amount, new Date().toLocaleString('ko-KR')], () => res.json({ success: true }));
        });
    });
});
...
io.on('connection', (socket) => {
    ...
    socket.on('send_message', (data) => { 
        db.run(`INSERT INTO chats (roomId, sender, senderPic, message, date) VALUES (?, ?, ?, ?, ?)`, 
            [data.roomId, data.sender, data.senderPic, data.message, new Date().toLocaleString('ko-KR')], () => { 
